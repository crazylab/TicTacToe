...
//-----------------------------------------------------------------------------
ttt.getMatrix = function(matrix,move,playerSymbol){
...
ttt.checkDiagonalMatch = function(player){
	var diagonal1 = matchEverything([11,22,33],player.moves);
	var diagonal2 = matchEverything([13,22,31],player.moves);
	return diagonal1 || diagonal2 ? player.symbol : null;
}
ttt.checkColumnMatch = function(player){
	...
	return column1 || column2 || column3 ? player.symbol : null;
}
ttt.checkRowMatch = function(player){
	...
	return row1 || row2 || row3 ? player.symbol : null;
}
ttt.checkForMatch = function(player){
	var diagonal = ttt.checkDiagonalMatch(player);
	var row = ttt.checkRowMatch(player);
	var column = ttt.checkColumnMatch(player);
